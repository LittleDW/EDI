/**
 * 作者：石奇峰
 * 功能：还款对账单匹配器，用于防止解析excel大文件造成的阻塞所做
...
let parsedData = XLSX.utils.sheet_to_json(sheet), headerRow = XLSX.utils.sheet_to_csv(sheet).split("\n")[0],
  ...
  predefinedHeaders = [
    ...
    "本金总期数", "利息总期数", "服务费总期数", "每期还款日", "利息总额（元）", "服务费总额（元）", "服务费结算月份", "还款日期",
    "当前期次", "应还本金（元）", "应还利息（元）", "应还总额（元）", "备注"
...
if (ngHeader) {
  let runner = (newMap,dbheader) => {
    ...
    return co(function* () {
      ...
      let [rows] = yield Query.call(connection, sqls.getOrgMode, getMySQLFieldValue({org_code, mode_type: "002"}));
      if (rows[0]) {
  ...
  promise = co(function* () {
    connection = yield getConnectionQ.call(pool)
    let Query = promisify(connection.query, {multiArgs: true});
    yield Query.call(connection, sqls.deleteOrgMode, getMySQLFieldValue({org_code, mode_type: "002"}));
  }).catch(function (err) {
    logger.error(err)
  }).then(() => {
    connection && connection.release();
  ...
  if (ngHeader) {
    ...
    parsedData.forEach(r => {
      let row = {
        ...
        repayment_date: getSheetValue(r["还款日期"]),
        current_period: getSheetValue(r["当前期次"]),
        repayment_principal_fee: getSheetValue(r["应还本金（元）"]),
      ...
      let new_fill_date = getSheetDateValue(row.fill_date),
        ...
        new_repayment_date = getSheetDateValue(row.repayment_date),
        new_current_period = phaseCount(row.current_period),
        new_repayment_principal_fee = getSheetNumberValue(row.repayment_principal_fee),
        new_repayment_interest_fee = getSheetNumberValue(row.repayment_interest_fee),
        new_repayment_all_fee = getSheetNumberValue(row.repayment_all_fee);
      ...
      if (!row.target_no && !row.target_name) {
        ...
        row._reason = "服务费率有误"
      } else if ((row.all_rate !== "") && (row.all_rate !== undefined) && (isNaN(new_all_rate) || (new_all_rate <= 0) || (new_all_rate < new_interest_rate))) {
        row._reason = "综合费率有误"
      } else if (((row.all_rate !== "") && (row.all_rate !== undefined)) && (row.service_rate !== "") && (row.service_rate !== undefined) && (Number(new_all_rate).toFixed(4) != Number(new_interest_rate + new_service_rate).toFixed(4))) {
        ...
        row._reason = "利息总期数有误"
      } else if ((row.service_fee_period_count !== "") && (row.service_fee_period_count !== undefined) && (isNaN(new_service_fee_period_count) || (new_service_fee_period_count < 0))) {
        ...
        row._reason = "每期还款日有误"
      } else if (isNaN(new_interest_total_fee) || (new_interest_total_fee <= 0)) {
        row._reason = "利息总额有误"
      ...
      } else if (!new_repayment_date) {
        row._reason = "还款日期有误"
      } else if (isNaN(new_current_period) || (new_current_period <= 0)) {
        row._reason = "当前期次有误"
      } else if (isNaN(new_repayment_principal_fee) || (new_repayment_principal_fee < 0)) {
        row._reason = "应还本金有误"
      } else if (isNaN(new_repayment_interest_fee) || (new_repayment_interest_fee <0)) {
        row._reason = "应还利息有误"
      } else if (isNaN(new_repayment_all_fee) || (new_repayment_all_fee <= 0)) {
        row._reason = "应还总额有误"
      }
      ...
      (row.repayment_date !== "") && (row.repayment_date !== undefined) && (row.repayment_date = new_repayment_date);
      (row.current_period !== "") && (row.current_period !== undefined) && (row.current_period = new_current_period);
      (row.repayment_principal_fee !== "") && (row.repayment_principal_fee !== undefined) && (row.repayment_principal_fee = new_repayment_principal_fee);
      (row.repayment_interest_fee !== "") && (row.repayment_interest_fee !== undefined) && (row.repayment_interest_fee = new_repayment_interest_fee);
      (row.repayment_all_fee !== "") && (row.repayment_all_fee !== undefined) && (row.repayment_all_fee = new_repayment_all_fee);

...
if (ngHeader) {
  let runner = (newMap) => {
    ...
    return co(function* () {
      ...
      let [rows] = yield Query.call(connection, sqls.getOrgMode, getMySQLFieldValue({org_code, mode_type: "006"}));
      if (rows[0]) {
  ...
  if (ngHeader) {
    ...
    parsedData.forEach(r => {
      let row = {
        ...
        order_status: getSheetValue(r["审核结果"]),
      ...
      } else if (isNaN(row.order_status) || !/^[1-2]$/.test(getSheetNumberValue(row.order_status))) {
        row._reason = "审核结果有误"
      }

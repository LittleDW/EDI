/**
 * 作者：石奇峰
 * 功能：服务费对账单匹配器，用于防止解析excel大文件造成的阻塞所做
 * 相似：放款对账单匹配器，还款对账单匹配器
 * */
...
let parsedData = XLSX.utils.sheet_to_json(sheet), headerRow = XLSX.utils.sheet_to_csv(sheet).split("\n")[0],
  ...
  predefinedHeaders = [
    ...
    "本金总期数", "利息总期数", "服务费总期数", "每期还款日", "利息总额（元）", "服务费总额（元）", "服务费结算月份", "结算日期",
    "当前期次","应结服务费（元）","备注"
...
if(ngHeader) {
  let runner = (newMap,dbheader)=>{
    ...
    return co(function*() {
      ...
      let [rows] = yield Query.call(connection, sqls.getOrgMode, getMySQLFieldValue({org_code, mode_type:"003"}));
      if(rows[0]){
  ...
  promise = co(function* () {
    connection = yield getConnectionQ.call(pool)
    let Query = promisify(connection.query, {multiArgs: true});
    yield Query.call(connection, sqls.deleteOrgMode, getMySQLFieldValue({org_code, mode_type: "003"}));
  }).catch(function (err) {
    logger.error(err)
  }).then(() => {
    connection && connection.release();
  ...
  if(ngHeader){
    ...
    parsedData.forEach(r => {
      let row = {
        ...
        service_settlement_date: getSheetValue(r["结算日期"]),
        current_period: getSheetValue(r["当前期次"]),
        current_service_fee: getSheetValue(r["应结服务费（元）"]),
        /*repayment_interest_fee: getSheetValue(r["应还利息（元）"]),
        repayment_all_fee: getSheetValue(r["应还总额（元）"]),*/
      ...
      let new_fill_date= getSheetDateValue(row.fill_date),
        ...
        new_service_settlement_date = getSheetDateValue(row.service_settlement_date),
        new_current_period = phaseCount(row.current_period),
        new_current_service_fee = getSheetNumberValue(row.current_service_fee);

      logger.info(row.service_total_fee)
      ...
      if (!row.target_no && !row.target_name) {
        ...
        row._reason = "计息天数有误"
      } else if ((row.interest_rate !== "") && (row.interest_rate !== undefined) && (isNaN(new_interest_rate) || (new_interest_rate<=0))) {
        row._reason = "借款利率有误"
      } else if (isNaN(new_service_rate) || (new_service_rate<=0)) {
        row._reason = "服务费率有误"
      } else if (isNaN(new_all_rate) || (new_all_rate<=0) || (new_all_rate < new_service_rate)) {
        row._reason = "综合费率有误"
      } else if (((row.interest_rate !== "") && (row.interest_rate !== undefined)) && (Number(new_all_rate).toFixed(4) != Number(new_interest_rate + new_service_rate).toFixed(4))) {
        row._reason = "综合费率须等于借款利率和服务费率之和"
      } else if ((row.repayment_mode !== "") && (row.repayment_mode !== undefined) && !/^00[1-8]$/.test(row.repayment_mode) && !predefinedRepaymentMode.includes(row.repayment_mode)) {
        row._reason = "本息还款方式有误"
      } else if (!/^00[1-6]$/.test(row.service_fee_mode) && !predefinedServiceFeeMode.includes(row.service_fee_mode)) {
        row._reason = "服务费结算方式有误"
      } else if ((row.principal_period_count !== "") && (row.principal_period_count !== undefined) && (isNaN(new_principal_period_count) || (new_principal_period_count <= 0))) {
        row._reason = "本金总期数有误"
      } else if ((row.interest_period_count !== "") && (row.interest_period_count !== undefined) && (isNaN(new_interest_period_count) || (new_interest_period_count <= 0))) {
        row._reason = "利息总期数有误"
      } else if (isNaN(new_service_fee_period_count) || (new_service_fee_period_count <= 0)) {
        row._reason = "服务费总期数有误"
      } else if (!new_each_repayment_day || (new_each_repayment_day < 1) || (new_each_repayment_day > 31)) {
        row._reason = "每期还款日有误"
      } else if ((row.interest_total_fee !== "") && (row.interest_total_fee !== undefined) && (isNaN(new_interest_total_fee) || (new_interest_total_fee <= 0))) {
        row._reason = "利息总额有误"
      } else if (isNaN(new_service_total_fee) || (new_service_total_fee <= 0)) {
        row._reason = "服务费总额有误"
      ...
      } else if ((row.service_settlement_date !== "") && (row.service_settlement_date !== undefined) && !new_service_settlement_date) {
        row._reason = "结算日期有误"
      } else if (isNaN(new_current_period) || (new_current_period <= 0)) {
        row._reason = "当前期次有误"
      } else if (isNaN(new_current_service_fee) || (new_current_service_fee <= 0)) {
        row._reason = "应结服务费有误"
      }
      ...
      (row.service_settlement_date !== "") && (row.service_settlement_date !== undefined) && (row.service_settlement_date = new_service_settlement_date);
      (row.current_period !== "") && (row.current_period !== undefined) && (row.current_period = new_current_period);
      (row.current_service_fee !== "") && (row.current_service_fee !== undefined) && (row.current_service_fee = new_current_service_fee);
    ...
    if(map){
      ...
      return co(function*() {
        ...
        let Query = promisify(connection.query, {multiArgs: true});
        let [rows] = yield Query.call(connection, sqls.getOrgMode, getMySQLFieldValue({org_code, mode_type:"003"}));
        return yield Query.call(connection, rows[0] ? sqls.updateOrgMode: sqls.insertOrgMode, getMySQLFieldValue({org_code, mode_type:"003", mode_content: JSON.stringify({_map:map,_headers:headers})}));
      }).catch(function (err) {
        logger.error(err)
      }).then(()=>{
        connection && connection.release();

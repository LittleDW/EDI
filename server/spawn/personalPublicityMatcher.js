...
let unmatched = [], matched = [], [file, extraParams] = process.argv.slice(2),
  ...
  asset_order_no_refer = [], borrow_name_refer = [], borrow_certificate_no_refer = [], borrow_phone_refer = [],
  borrow_card_no_refer = [], order_no_refer = [],
  queues = [], rawData = XLSX.utils.sheet_to_json(sheet);

rawData.forEach(r => {
  let row = {
    asset_order_no: getSheetValue(r["资产方订单号"]),
    borrow_name: getSheetValue(r["姓名"]),
    borrow_certificate_no: getSheetValue(r["身份证号"]),
    borrow_phone: getSheetValue(r["手机"]),
    borrow_card_no: getSheetValue(r["银行卡号"]),
  ...
  if (row.asset_order_no && asset_order_no_refer.includes(row.asset_order_no)){
    ...
    row._reason = "身份证号不能为空"
  } else if (!row.borrow_name){
    row._reason = "姓名不能为空"
  } else if (!row.borrow_phone){
    row._reason = "手机不能为空"
  } else if (!row.borrow_card_no){
    row._reason = "银行卡号不能为空"
  } else if (borrow_certificate_no_refer.includes(row.borrow_certificate_no)){
    row._reason = "重复的身份证号"
    ...
    borrow_certificate_no_refer.push(row.borrow_certificate_no);
    borrow_phone_refer.push(row.borrow_phone);
    borrow_card_no_refer.push(row.borrow_card_no);
    order_no_refer.push(row.order_no);
    if (!row.borrow_name && !row.borrow_certificate_no && !row.borrow_phone && !row.borrow_card_no && (row.asset_order_no || row.order_no)) {
      let connection;
      q = co(function* () {
        ...
        let [rows] = yield Query(sqls.ordersForPublicity, {order_no:row.order_no,asset_order_no:row.asset_order_no,...extras})
        if (!rows.length) {

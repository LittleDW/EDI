...
router.use((req, res, next) => {
  if(!req.session._submenu.includes("enterprise_publicity")){
    res.json({success:false, message:"您没有调用企业征信信息页面接口的权限"})
    return
  }
  next();
})
...
router.post('/search', (req, res,next) => {
  if(!req.session._button.includes("enterprise_publicity_search")){
    next();
    return
  }
  let connection
  (async () => {
    ...
    let [[countRows], [rows]] = await Promise.all([Query.call(connection, sqls.publicityEnterpriseSearchCount, params), Query.call(connection, sqls.publicityEnterpriseSearch, params)])
...
router.post('/create', (req, res, next) => {
  if(!req.session._button.includes("enterprise_publicity_create")){
    next();
    return
  }
  ...
  co(function* () {
    ...
    let queryCollection = yield [].concat.apply([],data.map(r=>{
        ...
        if (order_no){
          qs.push(Query(sqls.corpOrdersForPublicity, {order_no, ...userDif}).then(([rows])=>rows.map(j=>j.borrow_name)))
        }

        if (asset_order_no){
          qs.push(Query(sqls.corpOrdersForPublicity, {asset_order_no, ...userDif}).then(([rows])=>rows.map(j=>j.borrow_name)))
        }
    ...
    let [countCollection,[piCount]] = yield [
      queryData.map(r=>Query(sqls.countPublicityTaskDetail, {company_name:r,...param})),
      Query(sqls.countPublicityTasks, param)
    ];
    ...
    let piDetails, detailParam = queryData.map(r=>({company_name:r,...param, task_status: "1"})),
      piParam =  {...param, task_status: "1", total_count: queryData.length}
    try {
      yield Transaction()
      piDetails = yield [
        ...detailParam.map(r=>writeQuery(sqls.createPublicityTaskDetail, r)),
        writeQuery(sqls.createPublicityTask, piParam)
      ];
      yield Commit()
    } catch (e) {
      yield RollBack()
    ...
    if(ngDetail){
      throw new Error("插入企业征信信息爬取附表明细失败")
    }
    let pi = {...piParam};
    let row = yield ropAPI("ruixue.edi.enterprise.crawler.information", param)
    if(row.success){
      //yield writeQuery(sqls.updatePublicityTaskStatus, {...param, task_status: "2"})
      ...
      if(row.error_code !== 2){
        let statusUpdater = function* (count) {
          try {
            yield Transaction()
            yield [
              ...detailParam.map(r=>writeQuery(sqls.updatePublicityTaskDetailStatus, {...r, task_status: "3"})),
              writeQuery(sqls.updatePublicityTaskStatus, {...param, task_status: "3"})
            ];
            yield Commit()
          } catch (e) {
            yield RollBack()
    ...
    return co(function*() {
      ...
      let profilePart = {
        ...
        ...profilePart,
        oper_log: `动作：修改企业征信信息爬取主表 修改内容：增加任务：机构编码：${pi.org_code}，任务名：${pi.task_name}，任务状态：${pi.task_status}，查询数量：${pi.total_count}`
      }, detailLog = detail.map(r=>{
        r = operLogDictionaryGenerator('*', ['task_status'], r);
        return {
          from_table: "t_task_enterprise_pi_craw_detail",
          from_table_key: `${r.org_code},${r.task_name},${r.company_name}`,
          ...profilePart,
          oper_log: `动作：修改企业征信信息爬取辅表 修改内容：增加任务明细：企业名：${r.company_name}，机构编码：${r.org_code}，任务名：${r.task_name}，任务状态：${r.task_status}，`
        }
       })
      ...
      logger.info(err && err.stack || "记录企业信息查询操作任务发生错误")
    })
  }).catch(function (err) {
    logger.error(err)
    res.json({
...
router.post('/match', upload.any(), (req, res, next) => {
  ...
  co(function* () {
    let resultFile = yield spawning(path.resolve(`${__dirname}/../spawn/enterprisePublicityMatcher.js`), req.files[0].path,
...
router.post('/searchExport', (req, res,next) => {
  ...
  (async () => {
    ...
    let [rows] = await Query.call(connection, sqls.publicityEnterpriseSearchWithoutPagination, params);
    if (!rows || !rows[0]) {
      throw new Error("无记录")
    }
    res.json({

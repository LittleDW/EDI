var {getConnectionQ, getWriteConnectionQ,writeOperTableLog} = require('../pool'), sqls = require("../../config/sqls.json"),
  {logger, promisify, co, Router, oss, upload, fs, path, uuidv4, userDiffer,appendUUID, BusBoy, spawning,
    getMySQLFieldValue, promisifyInterval, removefileIfExist, promisifyPipingTempFile, thunkifyEvent, promisifyTimeout,
    connectToSocketHub,pushMessage
  } = require("../util"),
  ropAPI = require("../ropAPI"), configure = require('../../config/configure.json')[process.env.NODE_ENV],
  router = Router();
...
router.use((req, res, next) => {
  if(!req.session._submenu.includes("corp_orders")){
    res.json({success:false, message:"您没有调用企业订单列表页面接口的权限"})
    return
  }
  next();
})
...
router.post('/search', (req, res, next) => {
  ...
  co(function* () {
    ...
    let {assetOrderNo, orderNo, borrowDateStart, borrowDateEnd, createTimeStart, createTimeEnd, orderStatus, pageIndex, assetOrgCode, fundOrgCode} = req.body,
    ...
    let [[countRows], [rows]] = yield [
      Query.call(connection, sqls.corpOrdersCount, params),
      Query.call(connection, sqls.corpOrders, params)
    ]
    if (!countRows || !countRows[0]) {
      throw new Error("无记录")
    }
    res.json({success: true, rows, total: countRows[0].total, fee: countRows[0].fee})
    return rows
  }).catch(function (err) {
    res.json({
      success: false,
...
router.post('/repayment', (req, res, next) => {
  let connection
  co(function* () {
    connection = yield getConnectionQ(req)
    let Query = promisify(connection.query, {multiArgs: true});
    let [rows] = yield Query.call(connection, sqls.corpOrderRepayment, getMySQLFieldValue(req.body))
...
router.post('/voucher', (req, res, next) => {
  ...
  co(function* () {
    connection = yield getConnectionQ(req)
    let Query = promisify(connection.query, {multiArgs: true});
    let [rows] = yield Query.call(connection, sqls.corpOrderVoucher, getMySQLFieldValue(req.body))
...
router.post('/contract', (req, res, next) => {
  ...
  co(function* () {
    connection = yield getConnectionQ(req)
    let Query = promisify(connection.query, {multiArgs: true});
    let [rows] = yield Query.call(connection, sqls.corpOrderContract, getMySQLFieldValue(req.body))
    res.json({success: true, rows})
    return rows
  }).catch(function (err) {
    res.json({
...
router.post('/payment', (req, res, next) => {
  ...
  co(function* () {
    connection = yield getConnectionQ(req)
    let Query = promisify(connection.query, {multiArgs: true});
    let [rows] = yield Query.call(connection, sqls.corpOrderPayment, getMySQLFieldValue(req.body))
    res.json({success: true, rows, total: 0})
    return rows
  }).catch(function (err) {
    res.json({
...
router.post('/advance', (req, res, next) => {
  let connection
  co(function* () {
    connection = yield getConnectionQ(req)
    let Query = promisify(connection.query, {multiArgs: true});
    let [rows] = yield Query.call(connection, sqls.corpOrderAdvance, getMySQLFieldValue(req.body))
    res.json({success: true, rows, total: 0})
    return rows
  }).catch(function (err) {
    res.json({
...
router.post('/account', (req, res, next) => {
  ...
  co(function* () {
    connection = yield getConnectionQ(req)
    let Query = promisify(connection.query, {multiArgs: true});
    let [rows] = yield Query.call(connection, sqls.corpOrderAccount, getMySQLFieldValue(req.body))
    res.json({success: true, rows, total: 0})
    return rows
  }).catch(function (err) {
    res.json({
...
router.post('/filterOrderVoucher', (req, res, next) => {
  if(!req.session._button.includes("corp_orders_voucher_download") && !req.session._button.includes("corp_orders_voucher")){
    next();
    return
  }
  ...
  co(function* () {
    ...
    let [rows] = yield Query.call(connection, sqls.filterCorpOrderVoucher, params)
    res.json({success: true, rows})
    ...
    return co(function*() {
      let from_table = 't_order_voucher',
        ...
        action_type = '导出';
      let oper_log = `动作：导出企业订单资料 ${total}条, 导出参数 ${JSON.stringify(params)}`
...
router.get('/export', (req, res, next) => {
  ...
  if(!req.session._button.includes("corp_orders_export")){
    next();
    return
  }
  ...
  co(function* () {
    ...
    let {assetOrderNo, orderNo, borrowDateStart, borrowDateEnd, createTimeStart, createTimeEnd, orderStatus, fundOrgCode, assetOrgCode} = req.query,
      params = {...getMySQLFieldValue({
        assetOrderNo,
        orderNo,
        fundOrgCode,
    ...
    let [countRows] = yield Query.call(connection, sqls.corpOrdersCount, params)
    if (!countRows || !countRows[0]) {
      throw new Error("无记录")
    }
    ...
    for (var i = 0; i < pages; i++) {
      qList.push(spawning(path.resolve(`${__dirname}/../spawn/corpOrderAssist.js`), req.session.profile.user_type,
        assetOrderNo, orderNo, borrowDateStart, borrowDateEnd, createTimeStart, createTimeEnd, orderStatus,
        fundOrgCode, assetOrgCode, i * configure.exportLimit, configure.exportLimit, outputFile,
    ...
    yield Download.call(res, xslx, '企业订单导出.xlsx')
    ...
    let {params, total} = result
    return co(function*() {
      let from_table = 't_enterprise_order',
        from_org_code = req.session.profile.org_code,
        create_user_id = req.session.profile.user_id,
        create_sub_user_id = req.session.profile.sub_user_id,
        action_type = '导出';
      let oper_log = `动作：导出企业订单 ${total}条, 导出参数 ${JSON.stringify(params)}`
      return yield writeOperTableLog(writeConnection, {from_table, from_org_code, create_user_id, create_sub_user_id, oper_log, action_type})
    }).catch((err) => {logger.error(err)})
  }).then(() => {
    connection && connection.release();
...
router.post('/match', upload.any(), (req, res, next) => {
  if(!req.session._button.includes("corp_orders_create")){
    next();
    return
  }
  ...
  co(function* () {
    let resultFile = yield spawning(path.resolve(`${__dirname}/../spawn/corpAssetOrderMatcher.js`), req.files[0].path, req.files[1].path)
...
router.post('/create', (req, res, next) => {
  ...
  let busboy = new BusBoy({headers: req.headers, limits: {fieldNameSize: 200, fieldSize: 5242880}}),
    ...
    dataVisited = []/*,socketPromise = connectToSocketHub(req.sessionID,process.pid),socket*/;
  co(function* () {
          ...
          dataReady.forEach(r => {
            ...
            if (!dataVisited.includes(r)) {
              ...
              executorQueue.push(((r) => {
                ...
                return () => ropAPI("ruixue.edi.enterprise.order.apply.create", Object.assign({
                  ...
                  /*if(socket){
                    pushMessage(socket,{
                      ...
                      topic:"corporder-creating-progress",
...
router.post('/matchSupplement', upload.any(), (req, res, next) => {
  if(!req.session._button.includes("corp_orders_voucher_supplement")){
    next();
    return
  }
  if (!req.files || !req.files.length) {
  ...
  co(function* () {
    let resultFile = yield spawning(path.resolve(`${__dirname}/../spawn/supplementCorpAssetOrderMatcher.js`), req.files[0].path, req.files[1].path)
    let resultData = JSON.parse(fs.readFileSync(resultFile, {encoding: "utf8"}))
    res.json(Object.assign({success: true}, resultData))
    fs.unlinkSync(req.files[0].path)
    fs.unlinkSync(req.files[1].path)
...
router.post('/checkResultMatch', upload.any(), (req, res, next) => {
  ...
  let {mapper} = req.body, param = [path.resolve(`${__dirname}/../spawn/corpOrderCheckResultMatcher.js`), req.files[0].path, req.session.profile.org_code]
  if(mapper){
    param.push(mapper)
  }
  co(function*(){
...
router.post('/contractSupplymentMatch', upload.any(), (req, res, next) => {
  if(!req.session._button.includes("corp_orders_contract_supplement")){
    next();
    return
  }
  if (!req.files || !req.files.length) {
...
router.post('/accountDetailMatch', upload.any(), (req, res, next) => {
  ...
  let {mapper} = req.body, param = [path.resolve(`${__dirname}/../spawn/corpOrderAccountDetailMatcher.js`), req.files[0].path, req.session.profile.org_code]
  if(mapper){
    param.push(mapper)
  }
  co(function*(){

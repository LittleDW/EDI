...
const Dao = require('./enterpriseOrderDao');
...
const {promisifyInterval, promisifyPipingTempFile, promisifyTimeout, thunkifyEvent, logger, oss, appendUUID} = require('../../../util');
...
const orderExportService = async (params, req)=>{
  ...
  let { asset_order_no, order_no, borrow_date_start, borrow_date_end, create_time_start, create_time_end, order_status,
    fund_org_code, asset_org_code} = params
  let xslx = await pagingSpawningService(
    path.resolve(`${__dirname}/../../../spawn/corpOrderAssist.js`),
    [
      req.session.profile.user_type, asset_order_no, order_no, borrow_date_start, borrow_date_end,create_time_start,
      create_time_end,order_status,fund_org_code,asset_org_code,JSON.stringify(req.session.subUserDataRestriction || {}),
    ],
    pages
  )
  return {xslx, total}

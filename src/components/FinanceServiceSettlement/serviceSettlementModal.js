...
import FinanceServiceSettlementDetailServiceFeeModeSelector from '../Select/FinanceServiceSettlementDetailServiceFeeModeSelectorPage'
import {
  FormatFundAccountAccountPurpose,
  FormatFinanceServiceSettlementDetailInterestMode,
  FormatFinanceServiceSettlementDetailRepaymentMode,
  FormatFinanceServiceSettlementDetailServiceFeeMode,
} from "../Formatter";
...
class Modal extends Component {
  constructor(props) {
    ...
    this.xhr = null
    this.now = Date.now()
    this.handleFinanceDetailServiceFeeModeChange = this.handleFinanceDetailServiceFeeModeChange.bind(this)
    this.handleServiceSettlementAssetChange = this.handleServiceSettlementAssetChange.bind(this)
    this.handleServiceSettlementInputChange = this.handleServiceSettlementInputChange.bind(this)
    this.handleServiceSettlementChange = this.handleServiceSettlementChange.bind(this)
    this.handleServiceSettlementReset = this.handleServiceSettlementReset.bind(this)
    ...
    this.columns = [
      ...
      },
      {
        text: '计息方式',
        name: 'interest_mode',
        renderDom: row => (<FormatFinanceServiceSettlementDetailInterestMode value={row.interest_mode}/>)
      },
      {
        text: '计息天数',
      ...
      },
      {
        text: '本息还款方式',
        name: 'repayment_mode',
        renderDom: row => (<FormatFinanceServiceSettlementDetailRepaymentMode value={row.repayment_mode}/>)
      },
      {
        text: '服务费结算方式',
        name: 'service_fee_mode',
        required:true,
        renderDom: row => (<FormatFinanceServiceSettlementDetailServiceFeeMode value={row.service_fee_mode}/>)
      },
      {
        text: '本金总期数',
  ...
  componentWillReceiveProps(nextProps) {
    /*const {financeServiceSettlementDetail, working} = nextProps
    let {mapper} = this.state
    if(!working){
      let obj = {}
      if(financeServiceSettlementDetail && financeServiceSettlementDetail.ngHeaders && !mapper){
  ...
  handleServiceSettlementAssetChange(row) {
    this.setState({data: {...this.state.data,asset_org_code: row.value}, fundAccount: row.fundAccount, assetAccount: row.assetAccount})
  }
  handleServiceSettlementInputChange(input){
    return (e)=>{
      let value = e.target.value
      this.setState({data: {...this.state.data,[input]: (isNaN(value) || (value===""))? value : Number(value)}})
    }
  }
  handleServiceSettlementChange(e) {
    e.preventDefault();
    e.stopPropagation();
    var file = e.currentTarget.files[0]
    this.setState({serviceSettlementFile: file},()=>{
      this.submitBtn && this.submitBtn.focus()
      this.resetXHR()
    })
  }
  handleFinanceDetailServiceFeeModeChange(row){
    this.setState({data: {...this.state.data,settlement_mode: row.value}})
  }
  handleServiceSettlementReset(e){
    const {serviceSettlementFile} = this.state
    if(serviceSettlementFile){
      this.serviceSettlementButton.value = ""
      this.setState({serviceSettlementFile: null, mapper: null},this.resetXHR)
    }
  }
  resetXHR (runner){
  ...
  handleConfirm(e) {
    ...
    let {data, serviceSettlementFile, mapper} = this.state

    this.resetXHR(()=>{
      var formData = new FormData();
      formData.append("serviceSettlementFile", serviceSettlementFile);
        ...
        if(response && response.success){
          if(response.ngHeaders){
            ...
            this.resetXHR(()=>{
              ...
              formData.append("data", JSON.stringify({...data, settlement_details: JSON.stringify(response.matched)}))
              formData.append("settlementFile", serviceSettlementFile)
  ...
  toStepTwo(e){
    e.preventDefault();
    e.stopPropagation();
    let message = {}, {data,serviceSettlementFile} = this.state, self = this
    if ((typeof data.settlement_month != "string") || !data.settlement_month || !moment(data.settlement_month,"YYYY-MM").isValid()){
      message.settlement_month = "必填，格式为YYYY-MM"
    }
    if(!data.settlement_service_fee|| (data.settlement_service_fee < 0) || (data.settlement_service_fee > 1000000000)){
      message.settlement_service_fee = "必填，正数，且为不大于10亿"
    }
  ...
  backToStepOne(e){
    this.setState({step: 0, serviceSettlementFile: null},this.resetXHR)
  }
  ...
  renderFirstStep(){
    const {message, data, fundAccount,assetAccount, serviceSettlementFile} = this.state
      ...
      <div>
        ...
        <div className="form-group">
          ...
          <div className="col-sm-5">
            <AssetRelatedAccountPurposeWithAssetUserFromSelector onChange={this.handleServiceSettlementAssetChange} defaultValue={this.state.data && this.state.data.asset_org_code || ""} fundOrgCode={defaultFundOrgCode} assetAccountPurpose="001" fundAccountPurpose="003" userFrom="1" noEmpty={true}/>
          </div>
          {message.asset_org_code &&
            <div className="col-sm-3"><span className="help-block">{message.asset_org_code}</span></div>}
        </div>
        <div className="form-group">
          <label className="col-sm-4 control-label asterisk">服务费结算方式</label>
          <div className="col-sm-5">
            <FinanceServiceSettlementDetailServiceFeeModeSelector onChange={this.handleFinanceDetailServiceFeeModeChange} defaultValue={this.state.data && this.state.data.settlement_mode || ""} noEmpty={true}/>
          </div>
          {message.settlement_mode &&
            <div className="col-sm-3"><span className="help-block">{message.settlement_mode}</span></div>}
        </div>
        <div className="form-group">
          <label className="col-sm-4 control-label asterisk">服务费结算月</label>
          <div className="col-sm-5">
            <input type="text" value={data.settlement_month} placeholder={"格式：YYYY-MM"} onChange={this.handleServiceSettlementInputChange("settlement_month")}/>
          </div>
          {message.settlement_month &&
            <div className="col-sm-3"><span className="help-block">{message.settlement_month}</span></div>}
        </div>
        <div className="form-group">
          <label className="col-sm-4 control-label asterisk">应结服务费（元）</label>
          <div className="col-sm-5">
            <input type="number" value={data.settlement_service_fee} onChange={this.handleServiceSettlementInputChange("settlement_service_fee")}/>
          </div>
          {message.settlement_service_fee &&
          <div className="col-sm-3"><span className="help-block">{message.settlement_service_fee}</span></div>}
        </div>
        <div className="form-group">
          <label className="col-sm-4 control-label">备注</label>
          <div className="col-sm-5">
            <input type="text" maxLength="255" value={data.remark} onChange={this.handleServiceSettlementInputChange("remark")}/>
          </div>
          {message.remark &&
        ...
        <div className="form-group">
          <label className="col-sm-4 control-label asterisk">付款明细</label>
          <div className="col-sm-8">
            <label htmlFor={`${this.now}ServiceSettlement`} className="btn icon-btn btn-default pull-left">
              <i className="fa fa-upload"></i>{serviceSettlementFile ? '重新上传' : '上传明细'}</label>
            <input type="file" className="hidden" ref={(button) => {this.serviceSettlementButton = button;}} hidden
                   id={`${this.now}ServiceSettlement`} onChange={this.handleServiceSettlementChange} onClick={this.handleServiceSettlementReset}
                   accept=".xlsx, .xls, .csv"/>
            <a target="_blank" href="/服务费结算明细模板.xlsx" className="pull-left form-text edi-mr edi-ml">下载模板</a>
            {serviceSettlementFile && <div className="pull-left form-text">{serviceSettlementFile.name}</div>}
          </div>
        </div>
  ...
  renderSecondStep(){
    const {serviceSettlementFile, progress} = this.state
    return (
      <div>
  ...
  renderSecondStepSwitch(){
    const {financeServiceSettlementDetail} = this.props
    let {mapper} = this.state
    if(financeServiceSettlementDetail){
      if(mapper && Array.isArray(financeServiceSettlementDetail.ngHeaders)){
        return this.renderSecondStepMatcher()
      } else if (Array.isArray(financeServiceSettlementDetail.unmatched) && financeServiceSettlementDetail.unmatched.length){
  ...
  renderSecondStepMatcher(){
    const {financeServiceSettlementDetail} = this.props
    let ngHeaders = financeServiceSettlementDetail.ngHeaders.map(r=>({label:r, value:r})), {mapper} = this.state
    return [
      ...
      ...this.columns.map((r,i)=>{
        ...
        defaultValue = financeServiceSettlementDetail.ngHeaders.includes(defaultValue)?defaultValue:""
        return (
      ...
      (
        <div className="row" key="thirdStepBody">
          <div className="col-sm-8 col-sm-offset-2 scroller" style={{ height: '23rem' }}>
            <Datatable columns={this.errorColumns} rows={financeServiceSettlementDetail.unmatched} noPgaging={true}/>
          </div>
        </div>
      )
    ]

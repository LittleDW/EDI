...
import {FormatTaskEnterprisePICrawStatus, FormatOrgCode} from '../../Formatter'
...
export default class EnterprisePublicity extends Component {
  constructor(props) {
    super(props)
    this.state = {
      startDate: null,
      endDate: null,
      pageIndex: 1,
      orgCode: props._SESSION.user_type === 2 ? props._SESSION.org_code : props._SESSION.user_type === 1 ? props._SESSION.org_code : '',
    ...
    this.columns = [
      ...
      },
      {
        ...
        name: 'task_status',
        style: {'width': '120px', textAlign: 'center'},
        renderDom: row => <FormatTaskEnterprisePICrawStatus value={row.task_status} />
      },{
        text: '成功数量（条）',
        name: 'finish_count',
        style: {'width': '120px', textAlign: 'center'}
        ...
        name: 'task_status',
        style: {'width': '120px', textAlign: 'center'},
        renderDom: row => (row.task_status === 3 || row.task_status === '3') && row.down_url != '' && <a href={row.down_url} download>下载</a>
      }]
    ...
    this.searchBarSetting = [
      {
        label: "用户类型:",
        type:"custom",
        renderDom: () => <UserTypeSelector onChange={this.handleUserTypesChange} allowedType={defaultUserType}/>
      },
      {
        label: "机构:",
        type:"custom",
        renderDom: () => <OrgSelector onChange={this.handleOrgChange} allowedType={self.state.allowedType}/>
      },
      {
        label: "查询开始日:",
        type: "date",
  ...
  handleSearch = (index) => {
    this.setState({pageIndex: index ? index : 1}, () => {
      const {startDate, endDate, pageIndex, orgCode} = this.state
      this.props.search({
        startDate: startDate && startDate.format('YYYY-MM-DD 00:00:00'), endDate: endDate && endDate.format('YYYY-MM-DD 23:59:59'), pageIndex, taskName: this.nameInput && this.nameInput.value, orgCode
      })
    })
  }
  ...
  handleExport = e => {
    ...
    this.props.searchExport({
      startDate: startDate && startDate.format('YYYY-MM-DD 00:00:00'), endDate: endDate && endDate.format('YYYY-MM-DD 23:59:59'), taskName: this.nameInput && this.nameInput.value, orgCode
    }).promise.then(({response}) => {
      if(response.rows && response.rows.length> 0){
        fastExport(response.rows, this.exportColumns, '企业征信导出.xlsx');
      }
    })
  };
  render() {
      ...
      <div className="component">
        <SearchBar items={this.searchBarSetting} searcher={this.onSearch}>
          ...
          {Array.isArray(_buttons) &&
            _buttons.includes('enterprise_publicity_export') && (
              <button
                type="button"
          ...
          {Array.isArray(_buttons) && _buttons.includes('enterprise_publicity_create') && _SESSION.user_type !== 3 && <button type="button" className="btn icon-btn btn-warning pull-right" onClick={() => {navigateTo("searchEnterprisePublicityDetail")}}>
            <i className="fa fa-search"></i>生成报告
          </button>}
        </SearchBar>
        <div className="wrapper">
          <Datatable columns={this.columns} searcher={self.handleSearch} rows={data.rows} total={data.totalRow} index={this.state.pageIndex}  />
          <p className="wrapper__content-tips">共 <span style={{color: 'green'}}>{data.totalRow}</span> 条记录：查询数量 <span style={{color: 'green'}}>{data.totalCount}条</span>，成功 <span style={{color: 'red'}}>{data.success}</span> 条，  失败 <span style={{color: 'red'}}>{data.fail}</span> 条</p>
        </div>
      </div>
    )
  }

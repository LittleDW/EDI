...
import style from '../Orders/style.scss'
...
class Modal extends Component {
        ...
        csv = `\ufeff${this.successColumns
          ...
          .map(
            item =>
              "成功," +
              this.successColumns
                .map(col => (item[col.name] ? item[col.name] : ""))
                .join(",")
          )
          .join("\r\n")}`;
  ...
  renderUpload() {
    ...
    return (
      <div className="block finance-block">
        {progress && (
        ...
        <div className="form-group">
          <h4 className="col-sm-4 control-label">上传授信结果:</h4>
        </div>
        <div className="form-group">
          <label className="col-sm-4 control-label asterisk">授信结果</label>
          <div className="col-sm-8">
            <label
              htmlFor={`${this.now}repayment`}
              className="btn icon-btn btn-default pull-left edi-mr"
            >
              <i className="fa fa-upload" />
              {uploadFile ? "重新上传" : "上传授信结果"}
            </label>
            <input
              type="file"
              className="hidden"
            ...
            {uploadFile && (
              <div className="pull-left form-text edi-ml">
                {uploadFile.name}
              </div>
            )}
          </div>
  ...
  renderMatcher() {
    const { detail } = this.props;
    let ngHeaders = detail.ngHeaders.map(r => ({
        label: r,
        value: r
      })),
      { mapper } = this.state;
          ...
          <form className="modal-content form-horizontal">
            ...
            <div className="modal-footer">
              ...
              {message &&
                message.text &&
                message.type == "FAIL" && (
                  <p className="text-danger dialog-message">
              ...
              {message &&
                message.text &&
                message.type == "INFO" && (
                  <p className="text-info dialog-message">
                    <i className="fa fa-warning" /> {message.text}
                  </p>
                )}
              {message &&
                ...
                message.type == "SUCCESS" && (
                  <p className="text-success dialog-message">
                    <i className="fa fa-warning" /> {message.text}
                  </p>
                )}
            </div>

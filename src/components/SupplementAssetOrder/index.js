...
class SupplementAssetOrder extends Component {
  constructor(props) {
    super(props)
...
/*    if(!showMode && (newAssetOrders !== assetOrders)){
  ...
  exportResult(){
    ...
    if(createTab){
      if(assetOrders.failed.length < 400000) {
        fastExport([...assetOrders.failed,...assetOrders.unmatched], this.tableColumns, "资料补充失败记录.xlsx")
        return
      } else {
  ...
  renderHint(){
    ...
    let message
    if(progress){
      message = `${progress.percent} 已上传，${creating?'资料补充中...':''}`
    } else if (matching){
      message = "匹配中..."
          ...
          <section className="section__handleOrder-960">
            <form onSubmit={this.handleMatchAssetOrder} className="form-horizontal">
              <h3 className="form-group" style={{marginTop: '0'}}>
                <span className="col-sm-3 text-right">订单资料批量补充</span>
                {this.renderHint()}
              </h3>
              <hr />
              ...
              <div className="form-group row">
                <label className="col-sm-3 control-label asterisk">资料补充信息</label>
                <div className="col-sm-6">
                  <a target="_blank" href="/订单资料补充模板.xlsx" styleName="label-value">下载模板</a>
                  <p className="help-block edi_text-danger"><i className="fa fa-warning"></i> 请务必按照模板编辑资料，否则将可能资料补充失败</p>
        ...
        <a download="true" className="hidden" target="_blank" ref={(link) => {this.hiddenLink = link}}/>
        {
          ...
          <section ref={(section)=>{this.resultSection = section}} className="section__handleOrder-960">
            ...
            <ul className="nav nav-tabs">
              <li role="presentation" className={(createTab == 0)?'active':''} onClick={()=>{this.selectCreateTab(0)}}><a href="javascript:">资料补充成功结果</a></li>
              <li role="presentation" className={(createTab == 1)?'active':''} onClick={()=>{this.selectCreateTab(1)}}><a href="javascript:">资料补充失败结果</a></li>
            </ul>
...
export default SupplementAssetOrder

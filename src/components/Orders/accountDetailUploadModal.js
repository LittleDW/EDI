...
class Modal extends Component {
  ...
  handleExport = (e) => {
    ...
    if (tab === 1) {
      ...
      if (matched.length > 400000) {
        csv = `\ufeff${this.successColumns.map(item => item.text).join(',')}\r\n${matched.map(item => '成功,' + (this.successColumns.map(col => item[col.name] ? item[col.name] : '').join(','))).join('\r\n')}`
      ...
      title = `匹配失败结果`
      if (matched.length > 400000) {
        csv = `\ufeff${this.errorColumns.map(item => item.text).join(',')}\r\n${unmatched.map(item => this.errorColumns.map(col => item[col.name] ? item[col.name] : '').join(',')).join('\r\n')}`
        const blob = new Blob([csv], {type: "text/csv;charset=utf-8"});
        FileSaver.saveAs(blob, `${title}.csv`);
      } else {
  ...
  handleConfirm(e) {
    ...
    this.resetXHR(()=>{
      ...
      mc.promise.then(({response})=>{
        if(response && response.success && !response.ngHeaders && (!response.unmatched || !response.unmatched.length)){
          this.resetXHR(()=>{
            var formData = new FormData();
            formData.append("data", JSON.stringify({...data, details: JSON.stringify(response.matched)}))
            formData.append("uploadFile", uploadFile)
    ...
    return (
      <div className="block" styleName="finance-block">
        ...
        <div className="form-group">
          <h4 className="col-sm-4 control-label">上传出账明细:</h4>
        </div>
        <div className="form-group">
          <label className="col-sm-4 control-label asterisk">出账明细</label>
          <div className="col-sm-8">
            <label htmlFor={`${this.now}repayment`} className="btn icon-btn btn-default pull-left">
              <i className="fa fa-upload"></i>{uploadFile ? '重新上传' : '上传出账明细'}</label>
            <input type="file" className="hidden" ref={(button) => {this.fileButton = button;}} hidden
                   id={`${this.now}repayment`} onChange={this.handleFileChange} onClick={this.handleFileReset}
                   accept=".xlsx, .xls, .csv"/>
            <a target="_blank" href="/银行订单出账明细模版.xlsx" className="pull-left form-text" style={{marginLeft: '1rem'}}>下载模板</a>
            {uploadFile && <div className="pull-left form-text"style={{marginLeft: '1rem'}}>{uploadFile.name}</div>}
          </div>
        </div>
        <div className="form-group">
  ...
  renderMatcher(){
    const {detail} = this.props
    let ngHeaders = detail.ngHeaders.map(r=>({label:r, value:r})), {mapper} = this.state
    return [
        ...
        return (
          <div className="form-group" key={i}>
            ...
            <div className="col-sm-6">
              <Select onChange={this.mapperChanger(r.name)} noEmpty={true} options={ngHeaders} defaultValue={defaultValue}/>
            </div>
          </div>
        )
      })
  ...
  renderResult(){
    ...
    return [
      ...
      <div className="col-sm-8 col-sm-offset-2" styleName="scroller"  key={'result'}>
        <ul className="nav nav-tabs col-sm-12" role="tablist" key={'ul'}>
          <li role="presentation" className={(tab === 1) ? 'active' : ''} onClick={e => this.selectTab(e, 1)}><a href="javascript:;">匹配成功结果（{detail.matched?detail.matched.length:0}条）</a></li>
          <li role="presentation" className={(tab === 2) ? 'active' : ''} onClick={e => this.selectTab(e, 2)}><a href="javascript:;">匹配失败结果（{detail.unmatched?detail.unmatched.length:0}条）</a></li>
        </ul>
        {((tab === 1 && detail.matched && detail.matched.length > 0) || (tab === 2 && detail.unmatched && detail.unmatched.length > 0)) && <button type="button" className="btn btn-primary" onClick={this.handleExport} style={{'position': 'absolute', 'top': '0', 'right': '1rem'}}>导出</button>}
        {tab === 1 && <div key={'tab1'} style={{"width": "100%", 'clear': 'both'}}>
          <Datatable columns={this.successColumns} rows={detail.matched} noPgaging={true} style={{'overflow': 'auto', 'padding': '1rem 0'}}/>
        </div>}
        {tab === 2 && <div key={'tab2'} style={{"width": "100%", 'clear': 'both'}}>
          <Datatable columns={this.errorColumns} rows={detail.unmatched} noPgaging={true} style={{'overflow': 'auto', 'padding': '1rem 0'}}/>
        </div>}
      </div>
    ]
  }
        ...
        <div className="modal-dialog modal-lg" role="document">
          <form className="modal-content form-horizontal">
            ...
            <div className="modal-footer">
              <button type="submit" className={`btn btn-primary pull-right ${!this.state.enableConfirmBtn ? 'disabled' : ''}`} onClick={this.handleConfirm} ref={(submitBtn)=>{this.submitBtn = submitBtn}}>确定</button>

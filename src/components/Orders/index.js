...
import {getBrowserType, isMobile, downloadUrl} from '../../utils/etc'
import style from "./style.scss"
import CheckResultUploadModal from './checkResultUploadModal'
import ContractSupplymentModal from './contractSupplyment'
import AccountDetailUploadModal from './accountDetailUploadModal'
...
export default class Orders extends Component {
  constructor(props) {
          ...
          if (hasOpsColumn) {
            return (<div styleName="table-ops">
              ...
              <div styleName={`${row.order_no == self.state.opsOrderNo ? 'show-ops': 'hidden-ops'}`}>
                {_buttons.includes("orders_detail") &&
                <a href="javascript:" onClick={() => self.openOrderModal(row)}>订单</a>}
                {_buttons.includes("orders_advance") &&
                <a href="javascript:" onClick={() => self.openOrderAdvanceModal(row)}>垫付</a>}
                {_buttons.includes("orders_account") &&
                <a href="javascript:" onClick={() => self.openOrderAccountModal(row)}>出账</a>}
                {_buttons.includes("orders_payment") &&
                <a href="javascript:" onClick={() => self.openOrderPaymentModal(row)}>到账</a>}
                {_buttons.includes("orders_repayment") && <a href="javascript:" onClick={() => {
                  self.openOrderRepaymentModal(row)
                }}>兑付</a>}
                {_buttons.includes("orders_contract") && <a href="javascript:" onClick={() => {
                  self.openOrderContractModal(row)
                }}>合同</a>}
                {_buttons.includes("orders_service") && <a href="javascript:" onClick={() => {
                  self.openOrderServiceModal(row)
                }}>服务费</a>}
                {_buttons.includes("orders_voucher") && <a href="javascript:" onClick={() => {
                  self.openOrderVoucherModal(row)
                }}>资料</a>}
                {_buttons.includes("orders_credit") && <a href="javascript:" onClick={() => self.openOrderCreditModal(row)}>授信</a>}
    ...
    this.voucherColumns = [{
      ...
      renderDom: (row) => (<a href="javascript:" download target="_blank" onClick={() => self.downloadVoucher(row)}>下载</a>)
    }]
    this.allVoucherColumns = [{
      text: '资料名称',
      name: 'voucher_name',
      withTitle: true,
      style: {maxWidth: "200px", overflow: 'hidden', textOverflow: 'ellipsis'}
    ...
    this.contractColumns = [{
      ...
      //withTitle: true,
      style: {maxWidth: "600px", overflow: 'hidden', textOverflow: 'ellipsis'},
      renderDom: (row) => (<a href="javascript:" download target="_blank" onClick={() => self.downloadContract(row)}>下载</a>)
    ...
    this.orderColumns = [
      ...
      {text: '借款人名称', name: 'borrow_name'},
      {
        ...
        renderDom: row => <FormatOrderBorrowCertificateType value={row.borrow_certificate_type}/>
      },
      ...
      {text: '业务类型', name: 'business_type', renderDom: row => <FormatOrderBusinessType value={row.business_type}/>},
    ...
    this.accountColumns = [
      ...
      {text: '银行贷款合同号', name: 'bank_loan_contract_no', renderDom: ()=>{
          let selected = self.props.orders.orders.rows.find(r=>r.order_no == this.state.orderAccountNo);
          return selected && selected.bank_loan_contract_no || ''
        }},
      {text: '支付凭证地址', name: 'payment_certificate_url', renderDom: ()=>{
          let selected = self.props.orders.orderPayment.rows.find(r=>r.order_no == this.state.orderAccountNo);
    ...
    this.searchBarItems = [
      ...
      },
      {
        label: "借款期限:",
        type:"custom",
        dom: <DeadlineSelector onChange={this.handleDeadlineChange}/>
      },
      {
        label: "订单号:",
        type:"group",
        ...
        props:{
          items:[
            ...
            ,
            {
              ...
              type:"text",
              props:{
                ref:(input) => {this.borrowCertificateNoInput = input}
              },
              resetState: ()=>{
  ...
  handleDeadlineChange(row) {
    //const {deadLines} = this.props.orders
    let deadline_from = ''
    let deadline_to = ''
    if (row.value !== '') {
      deadline_from = row.from
      deadline_to = row.to
    }
    this.setState({deadline_from, deadline_to}, this.handleSearch)
  }
  handleSearch(index) {
      ...
      if(response.success){
        if(response.total <= window.EDI_CLIENT.exportMaxRows){
          var browserType = getBrowserType(),
            queryString = `/order/export?orderNo=${this.platformCodeInput && this.platformCodeInput.value ? this.platformCodeInput.value : ''}&assetOrderNo=${this.assetCodeInput && this.assetCodeInput.value ? this.assetCodeInput.value : ''}&fundOrgCode=${this.state.fundOrgCode}&assetOrgCode=${this.state.assetOrgCode}&borrowDateStart=${this.state.startDate && this.state.startDate.format('YYYY-MM-DD 00:00:00') || ''}&borrowDateEnd=${this.state.endDate && this.state.endDate.format('YYYY-MM-DD 23:59:59') || ''}&deadlineFrom=${this.state.deadline_from}&createTimeStart=${this.state.startCreateTime && this.state.startCreateTime.format('YYYY-MM-DD 00:00:00') || ''}&createTimeEnd=${this.state.endCreateTime && this.state.endCreateTime.format('YYYY-MM-DD 23:59:59') || ''}&deadlineTo=${this.state.deadline_to}&orderStatus=${this.state.status.join(",")}&borrowCertificateNo=${this.borrowCertificateNoInput && this.borrowCertificateNoInput.value || ''}`
  ...
  hasOpsColumn() {
    ...
    return Array.isArray(_buttons) && (_buttons.includes("orders_detail") || _buttons.includes("orders_advance")
      || _buttons.includes("orders_account") || _buttons.includes("orders_payment") || _buttons.includes("orders_repayment")
      || _buttons.includes("orders_contract") || _buttons.includes("orders_voucher"))
  }
  ...
  downloadVoucher = (row)=>{
    this.props.simplyLogIt({from_table:"t_order_voucher",from_table_key:`${row.order_no},${row.voucher_name}`,action_type:"个人订单页面资料下载",params:row}).promise.then(({response})=>{
      if(response && response.success){
        downloadUrl(row.voucher_url,this.hiddenLink)
      }
    })
  }

  downloadContract = (row)=>{
    this.props.simplyLogIt({from_table:"t_order_contract",from_table_key:`${row.order_no},${row.contract_type},${row.contract_number}`,action_type:"个人订单页面合同下载",params:row}).promise.then(({response})=>{
      if(response && response.success){
        downloadUrl(row.contract_url,this.hiddenLink)
      }
    })
  }

  render() {
    const {orders: ordersWrapper, navigateTo, type, orderRepaymentSearcher, orderPaymentSearcher, orderAdvanceSearcher, orderAccountSearcher, _buttons, checkResultMatcher, contractMatcher, accountDetailMatcher, contractSupplymentMatcher, contractSupplymentCreator, contractType} = this.props
    const {orderRepayment, orderPayment, orderAdvance, orderAccount, orders, orderCredit,contractList,voucherList,serviceList, allVouchers} = ordersWrapper
    const {showVoucher, showAllVoucher, confirmExport, confirmMassDownload, fundOrgCode, assetOrgCode,showCheckResultUploadModal, showAccountDetailUploadModal, orderAccountNo, showContractSupplymentModal} = this.state
    const self = this
      ...
      <div className="component">
        <SearchBar items={this.searchBarItems} searcher={this.onSearch}>
          <div className="pull-left" style={{width: '100px'}}>
            ...
            {(type === 2) && _buttons.includes("orders_contract_supplement") &&
          <button type="button" className="btn icon-btn btn-warning pull-right" style={{marginLeft: 0, width: '100px'}} onClick={this.openContractSupplymentModal}>
            <i className="fa fa-plus-circle"></i><span className="text">合同补充</span>
          </button>}
          </div>
          {(type == 2) && _buttons.includes("orders_check_result_upload") &&
          <button type="button" className="btn icon-btn btn-warning pull-right" onClick={this.openCheckResultUploadModal}>
            <i className="fa fa-upload"></i><span className="text">上传审核结果</span>
          </button>}
          {_buttons.includes("orders_export") &&
            <button type="button" className="btn icon-btn btn-danger pull-right" onClick={this.openExportConfirm}>
              <i className="fa fa-download"></i><span className="text">订单导出</span>
          </button>}
          {(type == 2) && _buttons.includes("orders_account_detail_upload") &&
          <button type="button" className="btn icon-btn btn-success pull-right" onClick={this.openAccountDetailUploadModal}>
            <i className="fa fa-cloud-upload"></i><span className="text">上传出账明细</span>
          </button>}
          {(type != 2) && _buttons.includes("orders_create") &&
            <button type="button" className="btn icon-btn btn-success pull-right" onClick={() => {navigateTo("createAssetOrder")}}>
              <i className="fa fa-plus"></i><span className="text">订单创建</span>
          </button>}
          {_buttons.includes("orders_voucher_download") &&
          <button type="button" className="btn icon-btn btn-danger pull-right" onClick={this.openMassDownloadConfirm}>
            <i className="fa fa-files-o"></i><span className="text">资料下载</span>
          </button>}
          {(type != 2) && _buttons.includes("orders_voucher_supplement") &&
          <button type="button" className="btn icon-btn btn-warning pull-right" onClick={() => {navigateTo("supplementAssetOrder")}}>
            <i className="fa fa-plus-circle"></i><span className="text">资料补充</span>
          </button>}
        </SearchBar>
        ...
        <div className="wrapper">
          <Datatable columns={column} rows={orders.rows} index={this.state.index} searcher={this.handleSearch}
                     total={orders.total}/>
          {orders.rows.length > 0 && <div styleName="summa">
            <p>共<span>{orders.total}条</span>记录；金额总计<span>{orders.fee}</span>元</p>
          </div>}
        </div>
        ...
        <Modal data={orderPayment.rows} title="到账明细一览" columns={this.paymentColumns} searcher={orderPaymentSearcher} closer={this.closeOrderPaymentModal}/>}
        {(orderAdvance.rows.length > 0) &&
        <Modal data={orderAdvance.rows} title="垫付明细一览" columns={this.advanceColumns} searcher={orderAdvanceSearcher} closer={this.closeOrderAdvanceModal}/>}
        {(orderAccount.rows.length > 0) &&
        <Modal data={orderAccount.rows} title="出账明细一览" columns={this.accountColumns} searcher={this.handleOrderAccountSearch} closer={this.closeOrderAccountModal}/>}
        ...
        <TableMode columns={this.contractColumns} data={contractList} title="合同一览" closer={this.closeOrderContractModal}/>}
        {serviceList && serviceList.length > 0 &&
        <TableMode columns={this.serviceColumns} data={serviceList} title="服务费一览" closer={this.closeOrderServiceModal}/>}
        {showVoucher && voucherList && voucherList.length > 0 &&
        <TableMode columns={this.voucherColumns} data={voucherList} title="资料一览" closer={this.closeOrderVoucherModal}/>}
        {showAllVoucher && allVouchers && allVouchers.length > 0 &&
        <VoucherTableMode columns={this.allVoucherColumns} data={allVouchers} title="订单搜索结果所含资料一览"
        ...
        <OperLogModal  tableName={'t_order'} tableKey={this.state.operLogOrderNo} title="个人订单日志"
                      closer={ this.hideOperLogModal } /> }
        {showContractSupplymentModal && <ContractSupplymentModal detail={ordersWrapper.detail} matcher={contractSupplymentMatcher} closer={this.closeContractSupplymentModal} confirm={this.handleContractSupplymentConfirm} working={ordersWrapper.working} contractType={contractType} title="合同补充"/>}
        {showCheckResultUploadModal && <CheckResultUploadModal detail={ordersWrapper.detail} matcher={checkResultMatcher} closer={this.closeCheckResultUploadModal} confirm={this.handleCheckResultConfirm} working={ordersWrapper.working} title="上传审核结果"/>}
        {showAccountDetailUploadModal && <AccountDetailUploadModal detail={ordersWrapper.detail} matcher={accountDetailMatcher} closer={this.closeAccountDetailUploadModal} confirm={this.handleAccountDetailConfirm} working={ordersWrapper.working} title="上传出账明细"/>}

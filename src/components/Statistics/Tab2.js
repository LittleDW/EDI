...
class Tab2 extends Component {
  ...
  renderTotalRate() {
    ...
    let val = sumArray(data, 'raise_fee') * 100 / sumArray(data, 'match_fee');
    if (isNaN(val)) {
      return '0.00%';
    } else {
  ...
  render() {
    ...
    const { dictionary, data, role, extValue, search } = this.props;
    const columnAll =
    ...
    return (
      <div className="wrapper__tab-area">
        <div className="row info-row" style={{width: '980px'}}>
          {this.props.children}
          <div className="col-sm-3">
            ...
            <div className="col-sm-8">
              <DeadlineSelector extValue={extValue} onChange={search} />
            </div>
          </div>
        </div>
        <div styleName="wrapper__tab-area__tab2">
          <div style={{marginRight: `${SCROLLWIDTH}px` }}>
            {data &&
              ...
              data.every(
                (item) =>
                  item[role === '1' ? 'fund_org_code' : 'asset_org_code'],
              ) && (
            ...
            {data &&
              ...
              data.every(
                ...
                <div
                  key="body"
                  style={{ maxHeight: '13.5rem', overflowY: 'scroll', marginRight: `-${SCROLLWIDTH}px` }}
                >
                  <table styleName="datatable">
                    <tbody>
                      {data.map((row, i) => (
                        <tr key={i}>
                          <td>{`${formatOrganizationCode(
                            row[
                              role === '1' ? 'fund_org_code' : 'asset_org_code'
                            ],
                            dictionary,
                          )}(${
                            row[
            ...
            {data &&
              ...
              data.every(
                ...
                <div
                  key="total"
                  style={{ maxHeight: '14.5rem', overflowY: 'scroll', marginRight: `-${SCROLLWIDTH}px` }}
                >
                  <table styleName="datatable">
                    <tbody>
...
export default CSSModule(Tab2, style, { allowMultiple: true });

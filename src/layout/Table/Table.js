...
class Table extends React.Component {
  ...
  handleBodyScrollLeft = (e) => {
    ...
    const { scroll = {} } = this.props;
    const { headTable, bodyTable, totalTable } = this;
    if (target.scrollLeft !== this.lastScrollLeft && scroll.x) {
      if (target === bodyTable) {
        headTable && (headTable.scrollLeft = target.scrollLeft);
        totalTable && (totalTable.scrollLeft = target.scrollLeft);
      } else if (target === headTable) {
        bodyTable && (bodyTable.scrollLeft = target.scrollLeft);
        totalTable && (totalTable.scrollLeft = target.scrollLeft);
      } else if (target === totalTable) {
        headTable && (headTable.scrollLeft = target.scrollLeft);
        bodyTable && (bodyTable.scrollLeft = target.scrollLeft);
      }
      this.setScrollPositionClassName();
  ...
  renderTable(options) {
    ...
    return [headTable, bodyTable, totalTable];
  }
